...
- Matching híbrido: filtro SQL determinista + ranking con LLM (Google Gemini 2.5 Flash, free tier).
...
| LLM | Google Gemini 2.5 Flash (free tier) | 1500 req/día gratis · uso comercial permitido · calidad comparable a Haiku |
...
| Google Gemini API | Scoring de subvenciones con LLM | **0€** dentro de free tier (1500 req/día, 1M tokens/día) | API key gratis |
| Brevo | Emails transaccionales + alertas | Free tier | API key |

### Coste mensual estimado

| Concepto | Mes 1-2 (~20 búsq/día) | Mes 3-6 (~100 búsq/día) | Si > 500 búsq/día |
|----------|------------------------|--------------------------|--------------------|
| Railway (web + Postgres) | 5€ | 5€ | 5€ |
| Gemini 2.5 Flash (3 batches/búsqueda) | 0€ (free tier) | 0€ (free tier) | ~5-10€ (al salir del free) |
| Brevo (free tier 300 emails/día) | 0€ | 0€ | 0€ |
| Dominio (~10€/año amortizado) | ~0.85€ | ~0.85€ | ~0.85€ |
| **Total** | **~6€/mes** | **~6€/mes** | **~11-16€/mes** |

El free tier de Gemini 2.5 Flash (1500 requests/día) cubre hasta ~500 búsquedas/día (3 batch calls cada una). La caché de scoring por `(empresa_perfil_hash, subvencion_id)` con TTL 7d reduce repeticiones aún más. Si revientas el free tier ya tienes el problema agradable de tener tráfico para monetizar; Gemini de pago es ~$0.075/M input tokens (más barato que Haiku).

**Fallback automático:** si Gemini cae o no responde en <5s, el sistema usa el score determinista del pre-rank (sin razón en lenguaje natural). El servicio nunca depende 100% del LLM.
...
└────────┬──────────────┬──────────────┬─────────┘
         ...
         ▼              ▼              ▼
   ┌──────────┐  ┌──────────────┐  ┌─────────────────┐
   │ Postgres │  │ Gemini Flash │  │ APIs externas   │
...
│   │   ├── scorer_llm.py      # ranking con Gemini 2.5 Flash
│   │   └── service.py
│   │
│   ├── alerts/
│   │   └── dispatcher.py
...
4. `POST /search` con form completo:
   ...
   - `matching.scorer_llm.score()` envía esos 30 candidatos a Gemini 2.5 Flash **en 3 llamadas batch (10 subvenciones por llamada)** vía `asyncio.gather`. El modelo devuelve para cada subvención `{score: int 0-100, razon: str}`.
...
| EU API cae | Idem |
| Gemini Flash falla o > 5s timeout | Fallback a scoring determinista por reglas (sin razón en lenguaje natural) |
...
- `scorer_llm`: cliente Gemini mockeado, verifica construcción del prompt y manejo de respuestas malformadas.
...
1. **API key de Google Gemini** (free tier) — alta gratis en https://aistudio.google.com/app/apikey, no requiere tarjeta.
...
| Coste de LLM se dispara con tráfico | Gemini free tier (1500 req/día) cubre ~500 búsquedas/día sin coste; pre-rank determinista deja solo top 30 candidatos; 3 llamadas batch por búsqueda; rate limit 60 búsquedas/hora por IP_hash; cache de scoring por `(empresa_perfil_hash, subvencion_id)` con TTL 7d |
...
9. **LLM:** Google Gemini 2.5 Flash para scoring (free tier; fallback a scoring determinista si falla). Se descarta Claude Haiku por coste — el plan Claude Max no cubre llamadas API programáticas.
